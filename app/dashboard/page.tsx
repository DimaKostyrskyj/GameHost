...
export default function Dashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#050507] text-white">
      <header className="sticky top-0 z-30 border-b border-zinc-900 bg-[#050507]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-4 sm:px-6">
          <a href="/" className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-black"><Gamepad2 size={19}/></span>
        ...
        <aside className="hidden min-h-[calc(100vh-64px)] w-64 shrink-0 border-r border-zinc-900 px-4 py-6 md:block">
          ...
          <nav className="mt-3 space-y-1">
            {tabs.map(item => {
              ...
              return <button key={item.id} onClick={() => openTab(item.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active ? "bg-white/[0.07] text-white" : "text-zinc-500 hover:bg-white/[0.03] hover:text-zinc-200"}`}><Icon size={18}/>{item.label}</button>;
            })}
          </nav>
          <div className="mt-8 rounded-2xl border border-indigo-500/10 bg-indigo-500/[0.04] p-4">
...
function Stat({ icon: Icon, label, value, hint }: { icon: typeof Server; label: string; value: string; hint: string }) {
  return <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6"><Icon className="text-zinc-500" size={21}/><p className="mt-5 text-sm text-zinc-500">{label}</p><p className="mt-1 text-2xl font-semibold">{value}</p><p className="mt-1 text-xs text-zinc-700">{hint}</p></div>;
}
...
function SettingsSection({ user }: { user: User }) {
  ...
  return <div className="max-w-3xl"><p className="text-sm text-zinc-600">Аккаунт</p><h1 className="mt-2 text-3xl font-semibold">Настройки</h1><p className="mt-2 text-sm text-zinc-600">Управление уведомлениями и безопасностью аккаунта.</p><div className="mt-8 space-y-4"><div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6"><div className="flex items-start justify-between gap-5"><div className="flex gap-4"><Bell className="mt-0.5 text-zinc-500" size={20}/><div><p className="font-medium">Уведомления</p><p className="mt-1 text-sm text-zinc-600">Получать уведомления о состоянии игровых серверов и важных событиях.</p></div></div><button onClick={() => setNotifications(v => !v)} className={`relative h-6 w-11 shrink-0 rounded-full transition ${notifications ? "bg-indigo-500" : "bg-zinc-800"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${notifications ? "left-6" : "left-1"}`} /></button></div></div><div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-6"><div className="flex gap-4"><ShieldCheck className="text-zinc-500" size={20}/><div><p className="font-medium">Безопасность</p><p className="mt-1 text-sm text-zinc-600">Аккаунт защищён сессией HttpOnly. Смена пароля будет добавлена следующим этапом.</p><p className="mt-4 text-xs text-zinc-700">Текущий аккаунт: {user.email}</p></div></div></div></div></div>;
}
